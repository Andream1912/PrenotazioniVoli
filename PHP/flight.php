...
if ($roundtrip == 'ritorno') {
    ...
    if (($endDate == "qualsiasi") || (empty($endDate))) {
        $sql_r = "SELECT * FROM volo WHERE citta_partenza = $1 AND citta_arrivo = $2" . $order;
...
<body>
    ...
    <form action="flight.php" method="get">
        ...
        <?php if (((isset($user)) && (!empty($user)) && ($endDate != "qualsiasi")) && ($from != 'ovunque' && $roundtrip != 'ritorno')) { ?>
            <div class="filter">
